...
\Meta{位置}は次の文字の組み合わせで指定：$\mathtt{t}=\mbox{ページ上部}$，
$\mathtt{b}=\mbox{ページ下部}$，$\mathtt{p}=\mbox{独立ページ}$，$\mathtt{h}=\mbox{その場}$，
$\mathtt{!}=\mbox{条件が悪くても指定位置に配置}$．
...
|\TeX|/|\LaTeX|/|\LaTeXe| & ロゴ\TeX/\LaTeX/\LaTeXe \\
\end{entable}
