...
$p'=0$ & |p'=0| & $a_1$ & |a_1| & $x^{2}$ & |x^2| & $\frac{3}{4}$ & |\frac{3}{4}|
  & $\sqrt[5]{6}$ & |\sqrt[5]{6}| \\
\mergecol{10}{\texttt{\bs begin\{tabular\}\{\Meta{列指定}\}}\enspace 行列用の表} \\
\end{entable}
...
\begin{entable}{2}{2}
|\deisplaystyle| & 別行立てスタイル & |\textstyle| & 本文スタイル \\
\end{entable}

書体の変更には |\mathrm|, |\mathbf|, |\mathit|,
|\mathsf|, |\mathtt|, |\mathmc|, |\mathgt| および |\mathnormal| が利用可（組み合わせは不可）．
英大文字に対してのみカリグラフ体への変更命令 |\mathcal| が利用可（例：$\mathcal{ABC}$）．
...
\begin{entable}[\enspace\quad]{2}{4}
$\times$ & |\times| & $\div$ & |\div| & $\land$ & |\land| & $\triangleright$ & |\triangleright| \\
$\cdot$ & |\cdot| & $\mp$ & |\mp| & $\lor$ & |\lor| & $\bigtriangleup$ & |\bigtriangleup| \\
...
$\oint$ & |\oint| & $\bigotimes$ & |\bigotimes| & $\biguplus$ & |\biguplus| & $\bigsqcup$ & |\bigsqcup| \\[3pt]
